...
JNIEXPORT jint JNICALL
Java_io_quartz_iouring_IoUring_getCqesBlocking(JNIEnv *env, jclass cls, jlong ring_address, jobject byte_buffer, jlong cqes_address, jint cqes_size ) {
    struct io_uring *ring = (struct io_uring *) ring_address;

    struct io_uring_cqe **cqes = (struct io_uring_cqe **) cqes_address;
  
    int32_t ret = io_uring_wait_cqe(ring, cqes);
    if (ret < 0) {
...
JNIEXPORT jint JNICALL
Java_io_quartz_iouring_IoUring_getCqes(JNIEnv *env, jclass cls, jlong ring_address, jobject byte_buffer, jlong cqes_address, jint cqes_size, jboolean should_wait, jlong timeoutMs) {
    struct io_uring *ring = (struct io_uring *) ring_address;
