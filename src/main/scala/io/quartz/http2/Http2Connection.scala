...
object Http2Connection {
  ...
  def make(
    ...
    for {
      ...
      runMe2 = outBoundWorkerProc(ch, outq, shutdownPromise)
        .handleErrorWith { e =>
          Logger[IO].debug("outBoundWorkerProc fiber: " + e.toString())
            *> c.closeTerminateSocket
            *> IO(true)
        }
        .iterateUntil(_ == true)
        .start
  ...
  def dataEvalEffectProducer(
    ...
    _ <- windowsUpdate(
      c,
      0,
      c.globalBytesOfPendingInboundData,
    ...
    _ <- windowsUpdate(
      c,
      streamId,
      stream.bytesOfPendingInboundData,
      stream.inboundWindow,
      len
    )
...
class Http2Stream(
    ...
    val inDataQ: Queue[
      IO,
      ByteBuffer
    ], // accumulate data packets in stream function
...
class Http2Connection(
  ...
  val headerDecoder =
    new HeaderDecoder(settings.MAX_HEADER_LIST_SIZE, settings.HEADER_TABLE_SIZE)

  val streamTbl =
    java.util.concurrent.ConcurrentHashMap[Int, Http2Stream](100).asScala
  ...
  def closeTerminateSocket = { terminateWrite.set(true) /*>> ch.close()*/ }

  def shutdown: IO[Unit] =
    outq.offer(null) >> shutdownD.get.void >> Logger[IO].debug(
  ...
  private[this] def updateWindowStream(streamId: Int, inc: Int) = {
    streamTbl.get(streamId) match {
      case None =>
        Logger[IO].debug(
          s"Update window, streamId=$streamId invalid or closed already"
        )
      case Some(stream) =>
        for {
  ...
  private[this] def handleStreamErrors(
      streamId: Int,
      e: Throwable
  ): IO[Unit] = {
    e match {
      case e @ ErrorGen(streamId, code, name) =>
        Logger[IO].error(
          s"handleStreamErrors: streamID = $streamId ${e.name}"
        ) >>
          ch.write(Frames.mkGoAwayFrame(streamId, code, name.getBytes))
            .void >> this.ch.close()
      case _ =>
        Logger[IO].error(s"handleStreamErrors:: " + e.toString) >> IO
          .raiseError(e)
    }
  }
  ...
  private[this] def openStream11(streamId: Int, request: Request): IO[Unit] = {
    for {
      ...
      _ <- Logger[IO].info(
        s"Open upgraded http/1.1 stream: $streamId  total = ${streamTbl.size} active = ${nS}"
      )

      d <- Deferred[
        IO,
        Headers
      ] // start stream, after done with headers and continuations
      done <- Deferred[IO, Unit]
      trailingHdr <- Deferred[
        IO,
        Headers
      ] // safe access to trailing header, only when they are fully ready
      ...
      _ <- streamFork
        .handleErrorWith(e => handleStreamErrors(streamId, e))
        .start
  ...
  private[this] def openStream(streamId: Int, flags: Int) =
    for {
      ...
      nS <- IO(concurrentStreams.get)
      _ <- Logger[IO].debug(
        s"Open stream: $streamId  total = ${streamTbl.size} active = ${nS}"
      )
      ...
      _ <- sendFrame(
        Frames.mkWindowUpdateFrame(streamId, INITIAL_WINDOW_SIZE - 65535)
      )
        .whenA(INITIAL_WINDOW_SIZE > 65535L)
      _ <- Logger[IO]
        .debug(
          s"Send UPDATE WINDOW, streamId = $streamId: ${INITIAL_WINDOW_SIZE - 65535}"
        )
        .whenA(INITIAL_WINDOW_SIZE > 65535L)
  ...
  private[this] def checkForTrailingHeaders(
      streamId: Int,
      flags: Int
  ): IO[Boolean] = {
    for {
      o <- IO(
        streamTbl.get(streamId)
      ) // if already in the table, we process trailing headers.
      trailing <- o match {
        case Some(e) =>
          IO
            .raiseError(
              ErrorGen(
                streamId,
                Error.PROTOCOL_ERROR,
                "A second HEADERS frame without the END_STREAM flag"
              )
            )
            ...
            IO
              .raiseError(
                ErrorGen(
                  streamId,
                  Error.PROTOCOL_ERROR,
                  "A second HEADERS frame on closed stream"
                )
              )
  ...
  private[this] def accumTrailingHeaders(
      streamId: Int,
      bb: ByteBuffer
  ): IO[Unit] =
    ...
    updateStreamWith(
      ...
      c =>
        for {
          ...
          _ <- c.inDataQ.offer(
            Frames.mkDataFrame(streamId, true, 0, ByteBuffer.allocateDirect(0))
          )
          http_headers <- IO(
            headerDecoder.decodeHeaders(c.trailing_header.toSeq)
          )
  ...
  private[this] def setEmptyTrailingHeaders(streamId: Int): IO[Unit] = {
    updateStreamWith(
      6,
      streamId,
      c => c.trailingHeader.complete(Headers()).void
    )
  }
      ...
      c =>
        for {
          ...
          _ <- IO
            ...
            .whenA(
              contentLenFromHeader.isDefined && c.contentLenFromDataFrames != contentLenFromHeader.get
            )
  ...
  private[this] def route(request: Request): IO[Response] = for {
    ...
    rep <- IO(
      Response
        .Ok()
        .asText("Hello World from Oleg Strigun")
  ...
  private[this] def route2(streamId: Int, request: Request): IO[Unit] = {
    ...
    val T = for {
      _ <- Logger[IO].debug(
        s"Processing request for stream = $streamId ${request.method.name} ${request.path} "
      )
      _ <- Logger[IO].trace(
        "request.headers: " + request.headers.printHeaders(" | ")
      )
      ...
      _ <- IO
        .raiseError(
          ErrorGen(
            ...
            "empty headers: COMPRESSION_ERROR"
          )
        )
      ...
      _ <- IO
        .raiseError(
          ErrorGen(
            streamId,
            Error.PROTOCOL_ERROR,
            "Upercase letters in the header keys"
          )
        )
      ...
      _ <- IO
        .raiseError(
          ErrorGen(
            streamId,
            Error.PROTOCOL_ERROR,
            "Connection-specific header field forbidden"
          )
        )
      ...
      _ <- IO
        .raiseError(
          ErrorGen(
            streamId,
            Error.PROTOCOL_ERROR,
            "TE header field with any value other than trailers"
          )
        )
        .whenA(
          request.headers
            .get("te")
            .isDefined && request.headers.get("te").get != "trailers"
        )
      ...
      _ <- response_o match {
        case Some(response) =>
          for {
            _ <- Logger[IO].trace(
              "response.headers: " + response.headers.printHeaders(" | ")
            )
            endStreamInHeaders <-
              if (response.stream == Stream.empty) IO(true) else IO(false)
            _ <- Logger[IO].trace(
        ...
        case None =>
          for {
            ...
            _ <- Logger[IO].trace(
              "response.headers: " + o44.headers.printHeaders(" | ")
            )
    ...
    T.handleErrorWith(e =>
      Logger[IO].error(e.toString()) *> this.shutdown
    )
  }
  ...
  def processIncoming(leftOver: Chunk[Byte]): IO[Unit] = (for {
    _ <- Logger[IO].trace(
      s"Http2Connection.processIncoming() leftOver= ${leftOver.size}"
    )
    _ <- Http2Connection
    ...
    case e @ TLSChannelError(_) =>
      Logger[IO].debug(
        s"connid = ${this.id} ${e.toString} ${e.getMessage()}"
      ) >>
    ...
    case e @ ErrorGen(streamId, code, name) =>
      Logger[IO].error(
        s"Forced disconnect connId=${this.id} code=${e.code} ${name}"
      ) >>
        ...
        if (len > settings.MAX_FRAME_SIZE)
          IO.raiseError(
            ErrorGen(
              ...
              "HEADERS exceeds SETTINGS_MAX_FRAME_SIZE"
            )
          )
          ...
          IO.raiseError(
            ErrorGen(
              streamId,
              Error.PROTOCOL_ERROR,
              "even-numbered stream identifier"
            )
          )
        else {
          frameType match {
              ...
              if (headerStreamId != streamId)
                ...
                for {
                    ...
                    case Some(t3) =>
                      IO.raiseError(
                        ErrorGen(
                          streamId,
                          Error.PROTOCOL_ERROR,
                          "HEADERS frame depends on itself"
                        )
                  ...
                  _ <- o_s match {
                    case Some(s) =>
                      IO.raiseError(
                        ErrorGen(streamId, Error.STREAM_CLOSED, "STREAM_CLOSED")
                      ).whenA(s.endFlag)
                    case None => IO.unit
                  }
                  trailing <- doStreamHeaders(streamId, flags)
                  _ <- Logger[IO]
                    .debug(s"trailing headers: $trailing")
                    .whenA(trailing == true)
                  ...
                  _ <- IO
                    .raiseError(
                      ErrorGen(
                        ...
                        "Second HEADERS frame without the END_STREAM flag"
                      )
                    )
                    .whenA(((flags & Flags.END_STREAM) == 0) && trailing)

                  _ <- accumHeaders(streamId, buffer).whenA(trailing == false)
                  _ <- accumTrailingHeaders(streamId, buffer).whenA(
                    trailing == true
                  )

                  _ <- markEndOfStream(streamId).whenA(
                    (flags & Flags.END_STREAM) != 0
                  )
                  _ <- markEndOfHeaders(streamId).whenA(
                    (flags & Flags.END_HEADERS) != 0
                  )

                  // if no body reset trailing headers to empty
                  _ <- setEmptyTrailingHeaders(streamId).whenA(
                    ((flags & Flags.END_STREAM) != 0) && trailing == false
                  )

                  _ <- triggerStream(streamId).whenA(
                    ((flags & Flags.END_HEADERS) != 0) && (trailing == false)
                  )

                  _ <- finalizeTrailingHeaders(streamId).whenA(
                    (flags & Flags.END_HEADERS) != 0 && trailing == true
                  )

                  _ <- IO { headerStreamId = 0 }.whenA(
                    (flags & Flags.END_HEADERS) != 0
                  ) // ready to tak new stream
            ...
            case FrameTypes.CONTINUATION =>
              ...
              for {
                ...
                _ <- IO
                  .raiseError(
                    ErrorGen(streamId, Error.PROTOCOL_ERROR, "END HEADERS")
                  )
                  .whenA(b1)
                _ <- markEndOfHeaders(streamId).whenA(
                  (flags & Flags.END_HEADERS) != 0
            ...
            case FrameTypes.DATA =>
              ...
              val padLen: Byte =
                if ((flags & Flags.PADDED) != 0) buffer.get() else 0
              ...
              for {
                ...
                t1: Long <- IO(
                  packet.size.toLong - padLen - Constants.HeaderSize - padByte
                )
                ...
                _ <- IO
                  .raiseError(
                    ErrorGen(
                      streamId,
                      Error.PROTOCOL_ERROR,
                      "DATA frame with invalid pad length"
                    )
                  )
                  .whenA(t1 != t2)
                ...
                _ <- markEndOfStreamWithData(streamId).whenA(
                  (flags & Flags.END_STREAM) != 0
                )
              } yield ()
            ...
            case FrameTypes.GOAWAY =>
              IO.raiseError(
                ErrorGen(
                  streamId,
                  Error.PROTOCOL_ERROR,
                  "GOAWAY frame with a stream identifier other than 0x0"
                )
              ).whenA(streamId != 0) >>
                IO {
                  ...
                  IO.raiseError(
                    ErrorGen(
                      streamId,
                      Error.PROTOCOL_ERROR,
                      "GOAWAY frame received " + new String(data)
                    )
                  )
                )
                ...
                _ <- IO
                  .raiseError(
                    ErrorGen(
                      streamId,
                      Error.PROTOCOL_ERROR,
                      "PRIORITY frame with 0x0 stream identifier"
                    )
                  )
                  .whenA(streamId == 0)
                _ <- IO
                  .raiseError(
                    ErrorGen(
                      streamId,
                      Error.FRAME_SIZE_ERROR,
                      "PRIORITY frame with a length other than 5 octets"
                    )
                  )
                  .whenA(len != 5)
                _ <- IO
          ...
          the size of all stream flow-control windows that it maintains by the
           difference between the new value and the old value.
            ...
            case FrameTypes.PUSH_PROMISE =>
              IO.raiseError(
                ErrorGen(streamId, Error.PROTOCOL_ERROR, "PUSH_PROMISE frame")
              )
              ...
              (for {
                ...
                _ <- IO
                  .raiseError(
                    ErrorGen(
                      streamId,
                      Error.PROTOCOL_ERROR,
                      "SETTINGS frame with ACK flag and payload"
                    )
                  )
                ...
                _ <- IO
                  .raiseError(
                    ErrorGen(
                      streamId,
                      Error.PROTOCOL_ERROR,
                      "SETTINGS frame with a stream identifier other than 0x0"
                    )
                  )
                  .whenA(streamId != 0)
                ...
                _ <-
                  if (Flags.ACK(flags) == false) {
                    for {
                      res <- IO(
                        Http2Settings.fromSettingsArray(buffer, len)
                      ) // <<<<<<<<<<<<<<<<<
                        .onError { case e: scala.MatchError =>
                          sendFrame(
                            Frames
                              .mkPingFrame(ack = true, Array.fill[Byte](8)(0x0))
                          )
                        }
                      ...
                      _ <- IO
                        ...
                        .whenA(
                          settings_done == true && res.MAX_FRAME_SIZE < settings_client.MAX_FRAME_SIZE
                        )
                      ...
                      _ <- IO
                        ...
                        .whenA(
                          (res.INITIAL_WINDOW_SIZE & Masks.INT32) > Integer.MAX_VALUE
                        )
                      ...
                      _ <- IO(this.settings.MAX_CONCURRENT_STREAMS =
                        this.MAX_CONCURRENT_STREAMS
                      )
                      _ <- IO(this.settings.INITIAL_WINDOW_SIZE =
                        this.INITIAL_WINDOW_SIZE
                      )

                      _ <- Logger[IO].debug(
                        s"Remote INITIAL_WINDOW_SIZE ${this.settings_client.INITIAL_WINDOW_SIZE}"
                      )
                      _ <- Logger[IO].debug(
                        s"Server INITIAL_WINDOW_SIZE ${this.settings.INITIAL_WINDOW_SIZE}"
                      )

                      _ <- sendFrame(
                        Frames.makeSettingsFrame(ack = false, this.settings)
                      ).whenA(settings_done == false)
                      ...
                      _ <- Logger[IO]
                        .debug(
                          s"Send UPDATE WINDOW global: ${INITIAL_WINDOW_SIZE - 65535}"
                        )
                        .whenA(INITIAL_WINDOW_SIZE > 65535L)
                ...
                case _: scala.MatchError =>
                  Logger[IO].error("Settings match error") >> IO.unit
                case e @ _ => IO.raiseError(e)
              }
