  ...
  def bufferToFrame(in: ByteBuffer): WebSocketFrame =
    if (in.remaining < 2 || FrameTranscoder.getMsgLength(in) < 0)
      ...
      val ret = opcode match {  
       case CONTINUATION => Continuation( Chunk.array( FrameTranscoder.decodeBinary(slice, m) ), finished )
       case BINARY => Binary( Chunk.array( FrameTranscoder.decodeBinary(slice, m) ) , finished )
       case TEXT   => Text( new String ( FrameTranscoder.decodeBinary(slice, m), StandardCharsets.UTF_8 ), finished )
       case CLOSE  => Close( Chunk.array( FrameTranscoder.decodeBinary(slice, m) ) )
       case PING   => Ping( Chunk.array( FrameTranscoder.decodeBinary(slice, m) ) )
       case PONG   => Pong( Chunk.array( FrameTranscoder.decodeBinary(slice, m) ) )    
      }
